...
To run the custom suite of unit test for Mailog use the following sequence of commands that will install dependencies
...
- The implementation should be done in Python 3.11+.
...
- Use Python docstrings with the `:args:`, `:return:`, etc. structure and with a newline after the docstring end (`"""`).
- Should try to use `asyncio` where possible, avoiding blocking calls (especially in I/O operations).
...
- Increment (look at `CHANGELOG.md` for semver changes) the `version` value in `setup.py`, then the `def _version(self):` return value in `src/mailog/main.py`, and then assert `app._version() ==` in `test/basic_test.py`.
...
Mailog is licensed under the [Apache License, Version 2.0](http://www.apache.org/licenses/).
